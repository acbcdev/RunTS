...
export function injectLogsIntoCode(
  ...
  try {
    ...
    const ast = parse(code, {
      ...
      ranges: true,
      onComment: () => {},
      // Evitar la recolección de comentarios si no se necesita
    });
    ...
    if (options?.injectLogs) {
      for (const p of insertPoints) {
        ...
        lines.splice(p.line, 1, logStatement);
      }
    }
...
function containsConsoleLog(node: ExpressionStatement): boolean {
  if (
