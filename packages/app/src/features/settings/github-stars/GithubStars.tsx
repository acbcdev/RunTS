...
function PureGithubStars() {
	const { data: stars, error } = useQuery<number | string>({
		queryKey: ["github-stars"],
		queryFn: async () => {
			const response = await fetch(
				"https://api.github.com/repos/acbcdev/runts",
			);
	...
	const displayStars = error ? "error fetching" : stars;
		...
		<a href="https://github.com/acbcdev/RunTS" target="_blank" rel="noreferrer">
			<div className="flex items-center justify-center gap-x-2 p-2 rounded-lg text-zinc-100 border  ">
				...
				<p className="space-x-0.5">
					...
					<span className="text-lg font-bold text-success">{displayStars}</span>
				</p>
